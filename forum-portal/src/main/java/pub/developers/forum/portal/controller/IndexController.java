...
import pub.developers.forum.portal.support.ResponseBodyAspect;
...
public class IndexController {
    ...
    public String index(IndexRequest request, Model model, HttpServletRequest servletRequest, HttpServletResponse response) {
        ...
        model.addAttribute("RecommendArticleList", RecommendArticleList(id));
    ...
    private List<Map<String, Object>> RecommendArticleList(Long id) {
        List<Map<String, Object>> postsList = new ArrayList<>();
        ResultModel<List<ArticleRecommendsResponse>> resultModel = articleApiService.recommends(10,id);
        if (!resultModel.getSuccess()) {
            return postsList;
        }
