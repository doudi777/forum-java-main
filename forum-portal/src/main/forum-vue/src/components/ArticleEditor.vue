...
<template>
  ...
  <div class="article-editor article-box" :style="{height: containerHeight}">
    ...
    <Modal
      title="润色结果"
      v-model="isShowPolishModel"
      :mask-closable="false">
      <div >
        <h3 class="tag-group">{{this.jsonData}}</h3>
      </div>
    </Modal>
    ...
    <div class="article-box-foot">
      ...
      <div class="article-box-foot-publish">
        <Button @click="polishArticle" type="info">润色</Button>
        <Button @click="updateArticle" type="info">保存并编辑</Button>
...
export default {
  ...
  methods: {
    ...
    polish (message, callback) {
      if (!this.preCheck()) {
        return
      }
      const headImgs = this.getHeadImgs()
      this.$Loading.start()
      this.$http.post(`/polish`, {
        id: this.article.id,
